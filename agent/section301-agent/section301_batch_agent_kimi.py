...
NOTE_LABEL = "note(20)"
OUTPUT_DIR = Path(__file__).resolve().parent / "output-kimi"
...
def _compare_llm_measures(
    ...
    kimi_measures: Sequence[Dict[str, Any]],
) -> Dict[str, Any]:
    openai_map = _index_measures(openai_measures)
    kimi_map = _index_measures(kimi_measures)
    base = _compare_measure_maps(openai_map, kimi_map, "openai", "kimi")
    return {
        "consistent": base["consistent"],
        "summary": base["summary"],
        "missing_in_openai": base["missing_in_left"],
        "missing_in_kimi": base["missing_in_right"],
...
class Section301KimiLLM:
    """Kimi LLM client for note(20) extraction."""
    ...
    def __init__(
        ...
        if OpenAI is None:  # pragma: no cover
            raise RuntimeError("kimi package required") from _OPENAI_IMPORT_ERROR
        self.model = model or os.getenv("MOONSHOT_MODEL", "kimi-k2.5")
        self.api_key = api_key or os.getenv("MOONSHOT_API_KEY")
        self.base_url = base_url or os.getenv("OPENAI_API_BASE", "https://api.moonshot.cn/v1")
        self.timeout = timeout

        if not self.api_key:
            raise RuntimeError("MOONSHOT_API_KEY required")
        self.client = OpenAI(api_key=self.api_key, base_url=self.base_url, timeout=self.timeout)
...
class Section301Note20Processor:
    ...
    def process(self, headings: Optional[Sequence[str]] = None) -> None:
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            kimi_future = executor.submit(self.kimi_llm.extract, note_text, context)
            openai_result = openai_future.result()
            try:
                kimi_result = kimi_future.result()
            except Exception:
                LOGGER.exception("Kimi request failed for %s; using OpenAI result", NOTE_LABEL)
                kimi_result = openai_result

        openai_payload = self._normalize_payload(openai_result, headings, hts_data)
        kimi_payload = self._normalize_payload(kimi_result, headings, hts_data)

        _write_output_json("note20", "openai", openai_payload)
        _write_output_json("note20", "kimi", kimi_payload)

        llm_compare = _compare_llm_measures(
            openai_payload["measures"],
            kimi_payload["measures"],
        )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        LOGGER.info("Initialized Kimi client with model: %s", args.kimi_model)
    except Exception as e:
        LOGGER.error("Failed to initialize Kimi client: %s", e)
        raise
