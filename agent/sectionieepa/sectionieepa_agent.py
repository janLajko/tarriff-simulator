...
C. Identify all **except** items:
   - Items explicitly listed after "except … provided for in …".
   - When the language says "As provided in heading 9903.01.32 ... shall not apply to products classified in the following subheadings", treat the enumerated HTS codes as the actual `except` entries and do not output heading 9903.01.32 (or similar referencing headings) itself.
   - Mark their source_label with "-exclusion".
   - Apply the same combination rule as scope items when conditions are identical.
D. Ignore carve-outs that clearly belong to other headings/programs (e.g., "As provided in 9903.01.26 ... products of Canada"). Unless the sentence explicitly ties the relief to `context.input_htscode`, do not add those references to `scope` or `except`.
E. If a sentence merely limits how to calculate the additional duties (e.g., "shall not apply to the declared value of the steel content of products provided for in headings 9903.81.89-9903.81.93, but shall apply to the non-steel content"), do NOT treat those headings as exclusions—they remain subject to the duties for their remaining content.
F. Set country_iso2 to the ISO-2 code for the country named in the text (e.g. "UK" for “products of the United Kingdom”). If no country is mentioned, leave it null.
G. Only populate effective_start_date and effective_end_date when explicit "effective" or date ranges appear in the text.  
...
J. If a supporting block explicitly states a different heading (e.g., "The rates … in heading 9903.81.89"), you may output its own scope entries but keep the source_label equal to that supporting note label.
